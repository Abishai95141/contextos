...
The Hooks Bridge is an HTTP server that receives lifecycle events from Claude Code and Cursor (via their hooks systems), enforces policies synchronously, records events asynchronously, and assembles Context Packs at session end. It is the runtime enforcement layer of ContextOS — every tool use by an AI coding agent passes through it before being executed.

Claude Code connects via HTTP hooks directly. Cursor connects via a command-based adapter script (`.cursor/hooks/contextos.sh`) that translates stdin/stdout JSON to HTTP calls. Both agents reach the same endpoints with the same semantics. See `docs/SYSTEM-DESIGN.md` Section 15 for the full Cursor adapter specification.
...
```
Claude Code (running locally)          Cursor (running locally)
         │                                     │
         │  HTTP POST (direct)                  │  stdin JSON → contextos.sh → HTTP POST
...
│  │  - Agent identity matching (agent_type)      │    │
│  │  - Priority ordering                         │    │
│  │  - Default allow                             │    │
│  └──────────────────────────────────────────────┘    │
│                                                      │
...
Both Claude Code and Cursor call hooks that result in HTTP POST requests to the Hooks Bridge. Claude Code sends HTTP POSTs directly; Cursor sends stdin JSON to the adapter script which forwards as HTTP POSTs. The Hooks Bridge must respond with `2xx` within 10 seconds. Non-2xx responses cause the agent to log an error and continue (non-blocking). Returning a JSON body with `decision: 'deny'` (on PreToolUse) blocks the tool use.
...
The Hooks Bridge must never block the agent's main thread. Policy evaluation must complete in under 500ms. If policy evaluation times out, default to `allow` and log the timeout.

### Cursor Hook Format

Cursor hooks are command-based (stdin/stdout JSON), not HTTP. ContextOS uses an adapter script (`.cursor/hooks/contextos.sh`) that:
1. Reads JSON from stdin (Cursor's hook input)
2. Normalizes field names: `conversation_id` → `session_id`, adds `agent_type: "cursor"`
3. POSTs the normalized payload to the Hooks Bridge HTTP endpoint
4. Translates the HTTP response to Cursor's expected stdout JSON format
5. Exits with code 0 (allow), 2 (deny), or 1 (error, fail-open)

**Field normalization table:**

| Concept | Cursor Field | Claude Code Field | Hooks Bridge Canonical |
|---------|-------------|-------------------|------------------------|
| Session ID | `conversation_id` | `session_id` | `session_id` |
| Hook type | `hook_event_name` | `hook_event_name` | `hook_event_name` |
| Tool name | `tool_name` | `tool_name` | `tool_name` |
| Tool input | `tool_input` (object) | `tool_input` (string) | `tool_input` (object) |
| Agent identity | `composer_mode` | N/A | `agent_type` |

The adapter script is generated by the VS Code extension (Feature Pack 07). See `docs/SYSTEM-DESIGN.md` Section 15 for the complete script.
...
```
Input: eventType, toolName, toolInput, projectId, agentType
...
2. For each rule in order:
   a. If rule.agent_type != agentType AND rule.agent_type != '*': SKIP
   b. If rule.event_type != eventType AND rule.event_type != '*': SKIP
   c. If rule.tool_pattern AND NOT globMatch(toolName, rule.tool_pattern): SKIP
   d. If rule.path_pattern:
   ...
   e. MATCH: return rule.decision + rule.reason

3. No rule matched: return { decision: 'allow', reason: 'default allow' }
```

**Agent identity values:** `claude_code`, `cursor`, `copilot`, `*` (wildcard). The `agentType` is determined from the request: Claude Code hooks include no explicit agent type (default to `claude_code`); Cursor hooks include `agent_type: "cursor"` via the adapter script normalization.
...
3. Calls the Semantic Diff service at `SEMANTIC_DIFF_URL/analyze` with the raw diff. **Note:** `POST /analyze` returns AST-only structural diff synchronously; LLM enrichment runs asynchronously via BullMQ (see Feature Pack 06 spec).
4. Formats a Context Pack in markdown:
   ...
   {AST-only structural diff; LLM summary added asynchronously when available}
...
## 7. Auto-Configuration of Agent Hook Settings

When a user registers a project with ContextOS (via the web app or VS Code extension), hook configuration files are generated for both Claude Code and Cursor.

### Claude Code: `.claude/settings.json`

The Hooks Bridge URL is auto-written to the project's `.claude/settings.json`.
...
### Cursor: `.cursor/hooks.json` + `.cursor/hooks/contextos.sh`

The VS Code extension generates two files:

1. **`.cursor/hooks.json`** — Registers the adapter script for all relevant hook events:
```json
{
  "version": 1,
  "hooks": {
    "sessionStart": [{ "command": ".cursor/hooks/contextos.sh" }],
    "preToolUse": [{ "command": ".cursor/hooks/contextos.sh" }],
...
2. **`.cursor/hooks/contextos.sh`** — The adapter script that normalizes Cursor's stdin JSON to the Hooks Bridge HTTP format and translates responses back. See `docs/SYSTEM-DESIGN.md` Section 15 for the complete script.

**Key design decisions:**
- `beforeMCPExecution` uses `failClosed: true` — MCP tool execution is security-critical; if the policy check fails, the tool must not run.
- All other hooks fail-open (default) — a hooks-bridge outage should not block the developer's IDE session.
- `preToolUse` is included alongside `beforeShellExecution`/`beforeMCPExecution` to capture generic tool events (Read, Write, Grep, Delete).
